...
- 仅在 **aiocqhttp** 平台可用
- 默认金句已内置完整列表，可直接在插件设置中增删改、批量导入
- `quotes_files` 仍可作为外部金句文件的补充来源（可选）；相对路径以 AstrBot 启动目录为基准
- 所有金句会合并、去重后随机抽取一条

> 若升级后未自动获得完整默认金句，可在设置中点击 `quotes` 的“批量导入”，将 `default_quotes.json` 内容导入即可。
...
- `telegram`：发送折叠引用块
- 支持自定义转发节点昵称（QQ 路径）
- QQ 合并转发会按 `node_max_length` 附近的自然断点拆成多个节点，并用 `node_hard_limit` 限制单个节点的最大长度，避免单个节点过长导致发送失败
